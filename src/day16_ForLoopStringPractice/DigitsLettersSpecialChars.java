...
public class DigitsLettersSpecialChars {
    ...
    public static void main(String[] args) {
        ...
        String str = "Cydeo1234Scholl!@&$%WoodenSpoon";
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (ch >= '0' && ch <= '9') { //if character is between '0'to'9' then it's digit
                digits += ch;
            } else if (ch >= 'A' && ch < 'Z') {// if the character is between 'A' to 'Z' then it's letter
                letters += ch;
            } else if (ch >= 'a' && ch <= 'z') {// if the character is between 'a' to 'z' then it's letter
                letters += ch;
            } else {// if the character neither digit nor letter,then it's special charecter
                specialChars += ch;
